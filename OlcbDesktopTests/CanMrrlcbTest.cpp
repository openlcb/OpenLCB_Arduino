...
LinkControl linkCtl(&txBuffer, &nodeid);
...
void store(){}
PCE pce(events, evtNum, &txBuffer, &nodeid, pceCallback, store, &linkCtl);
...
void loop() {
  ...
  if (rcvFramePresent) {
    ...
    linkCtl.receivedFrame(&rxBuffer);
  }

  // if link is initialized, higher-level operations possible
  if (linkCtl.linkInitialized()) {
...
TEST(CanMrrlcbTest, Main) {
    
    // printf("\n\n%s\n\n", getwd(0)); showed that CWD is the OlchDesktopTests directory itself
    // change output to run into a file, then manually diff it
    // (or use a large string and compare to file?)
    // or setup pipe I/O, except to a file?
